...
// internal/panelcarousel/draw.go drawCarouselColumn: both compute a job suffix, new-file tier,
// rename mark, and subtree mark and pack them into a RowSuffix, but how each caller derives
// those values differs (name-width gating, active-column gating, closure vs direct
// lookup), so that part stays at the call site.
func NewRowSuffix(job JobSuffix, tier NewFileMarkTier, renameMark, subtreeMark bool) RowSuffix {
	return RowSuffix{
		JobSuffix:        job,
		NewFileTier:      tier,
