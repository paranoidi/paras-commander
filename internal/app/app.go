...
func (a *App) fileDialogFocusCount() int {
	...
	return len(a.model.FileDialog.Fields) + 2 // fields + OK + Cancel
}

// fileDialogOnButton returns true if current focus is on a button (not a field).
func (a *App) fileDialogOnButton() bool {
	...
	return d.FocusedField >= len(d.Fields)
}
...
func (a *App) fileDialogFocusButton(delta int) {
	...
	// Fields + OK + Cancel: move between OK/Cancel only
	total := len(d.Fields)
	if d.FocusedField == total && delta == 1 {
		d.FocusedField = total + 1 // OK -> Cancel
	} else if d.FocusedField == total+1 && delta == -1 {
		d.FocusedField = total // Cancel -> OK
	}
	// Otherwise stay
}
