...
func (a *App) reconcileImageBeforeShow(plan *previewpanel.ImagePlacement) (forceShow bool) {
	...
	if a.image.lastSet {
		...
		// Kitty a=T with the same image id updates placement *data* but keeps positions.
		// Delete when clearing or when the placement moves/resizes; same-geometry
		// replacement (file→file in one pane) stays delete-free to avoid flicker.
		geometryChanged := plan == nil ||
			plan.X != a.image.last.X ||
			plan.Y != a.image.last.Y ||
			cols != a.image.lastCols ||
			rows != a.image.lastRows
