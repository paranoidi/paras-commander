...
| `progress_lightbar` | bool | `true` | Animate a moving highlight over the menu-bar job progress bar (colours from the theme's `menu.progress.done.gfx.*` keys). A job whose totals are still unknown (initial scan) shows this as a bouncing indeterminate bar instead of a filled cutoff. While the animated bar is on screen the menu-bar activity spinner is hidden (the bar replaces it); the spinner still appears for disk-usage scans, running commands and directory loads. |
