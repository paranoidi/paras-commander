...
type ActionSpec struct {
	...
	Title string
	// Section groups actions in the help screen (e.g. "File").
	Section string
...
// HelpSectionOrder is the curated section display order for the F1 help dialog's browse
// mode (empty filter query). It is the single source of truth for section grouping order;
// a Section value absent from this list (e.g. a future addition here in specs.go that this
// list wasn't updated for) falls back to alphabetical order after the known sections rather
...
// HelpSectionPriority maps each HelpSectionOrder entry to its position, for sorting help
// entries into section-contiguous order. A Section absent from this map (i.e. not in
// HelpSectionOrder) is not present as a key; callers must handle that case themselves.
var HelpSectionPriority = func() map[string]int {
	m := make(map[string]int, len(HelpSectionOrder))
	for i, s := range HelpSectionOrder {
		m[s] = i
	}
	return m
}()
