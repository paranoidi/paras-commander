...
- **`internal/apphandler/*`**: feature handlers (`jobs`, `find`, `commands`, `compare`, `dedup`; `dialog` reserved for future path-picker / transfer extraction). Each handler takes a `Deps` struct and a `Host` interface for cross-cutting app services. `commands` owns the Commands view (run-command list screen), the command-output dialog, and the run-for-each dialog/batch backend.
...
- **`internal/app/shell_host.go`**: shared `appShellHost` embedded by `jobs_host.go`, `find_host.go`, and `commands_host.go`.
