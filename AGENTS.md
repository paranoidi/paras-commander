...
- **`internal/app/shell_host.go`**: shared `appShellHost` embedded by `jobs_host.go`, `find_host.go`, `compare_host.go`, `meta_host.go`, `commands_host.go`, `preview_host.go`, and `dialog_host.go`. Each of those files implements one apphandler `Host` interface (6 to 32 methods) via one-line forwarders onto `App`; none is trivial enough (≤2 forwarders, or embed-only) to fold into `shell_host.go` without just relocating the same method count into a single oversized file, so each stays a separate file — one per handler.
