...
| `unicode_placeholder_terminals` | array of strings | `[]` | Extra `client_termtype` substrings (matched case-insensitively under tmux, in addition to the always-on `kitty`/`ghostty`) to treat as supporting Kitty's Unicode-placeholder image display. Use this to opt in a terminal like WezTerm where placeholder support is a build-specific capability `client_termtype` can't confirm on its own — e.g. `["wezterm"]`, only once you've verified your attached build actually supports it. |
...
| `image_max_edge_px` | int | `0` | Longest edge (px) for decoded stills, for protocols/contexts that don't need the tmux-Sixel safety clamp below (Kitty in any tmux state, or Sixel outside tmux). `0` (default) means unrestricted: decode at native resolution and let the final cell-budget fit determine the displayed size. Applied even when `prefetch` is `false`; when set above `0`, clamped to a minimum of 64. |
| `tmux_sixel_max_edge_px` | int | `1024` | Longest edge (px) for decoded stills when using Sixel under tmux, and for video-thumb grids in every protocol/context. Keeps tmux graphics payloads under its ~1MB input-buffer limit — Sixel under tmux is transmitted as one unchunked escape sequence, unlike Kitty's chunked payloads. Applied even when `prefetch` is `false` (minimum 64, no unrestricted option). |
