...
func DrawFilePreviewThemePicker(screen tcell.Screen, rect geom.Rect, state FilePreviewThemePickerState, styles theme.Theme) {
	...
	draw.DrawScrollingDialogInput(screen, primaryCol, rect.Y+1, inputWidth, draw.ScrollingInputState{Value: state.Query, Cursor: state.QueryCursor, Scroll: state.QueryScroll, LeadingIcon: styles.IconSearchIcon()}, true, false, styles)
