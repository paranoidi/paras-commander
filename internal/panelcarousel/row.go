...
func formatBriefRow(entry localfs.Entry, width int, showIcons bool, jobMarkRune rune, subtreeMark bool) string {
	iconStrip := 0
	if showIcons {
		iconStrip = 2
	}
	...
	rowTextWidth := width - leftGutter - iconStrip
	if rowTextWidth < 1 {
		rowTextWidth = 1
	}
	...
	return fmt.Sprintf("%-*s %*s", nameWidth, name, listSizeCells, formatListedSize(entry))
}

func formatListedSize(entry localfs.Entry) string {
...
func briefHeader(nameTitle, sizeTitle string, rowTextWidth int) string {
	...
	if len(nameTitle) > nameWidth {
		nameTitle = string([]rune(nameTitle)[:nameWidth])
	}
	if len(sizeTitle) > listSizeCells {
		sizeTitle = sizeTitle[:listSizeCells]
	}
