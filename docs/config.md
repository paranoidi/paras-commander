...
| `image_preview_debounce_ms` | int | `500` | Replaces `key_repeat_debounce_ms` when the coalesced quick-view/carousel preview target is an image or a video/audio file — those previews re-emit a sixel/Kitty payload to the terminal, which is far more expensive than a text reload. Only selects the delay: `key_repeat_debounce_ms = 0` still disables debouncing for every target. Values above 10000 are clamped. Edited alongside `key_repeat_debounce_ms` in **Options → Calibrate Debounce**. |
...
| `selection_size_scan_debounce_ms` | int | `100` | Delay after the panel selection, Find dialog marked selection, or an open delete-confirmation dialog's source selection changes before checking which directories still need a background disk-usage scan (stats each one to check mount boundaries). Runs off the main goroutine so a large selection change (e.g. select-all across thousands of directories) doesn't block input handling. `0` runs on the next scheduler tick; values above 30000 are clamped. |
...
| `progress_lightbar` | bool | `true` | Animate a moving highlight over the menu-bar job progress bar (colours from the theme's `menu.progress.done.gfx.*` keys, or `menu.progress.delete.gfx.*` while a move removes its sources or for delete jobs). A job whose totals are still unknown (initial scan) shows this as a bouncing indeterminate bar instead of a filled cutoff. While the animated bar is on screen the menu-bar activity spinner is hidden (the bar replaces it); the spinner still appears for disk-usage scans, running commands and directory loads. |
...
| `image_metadata` | string | `"essentials"` | Detail level for the EXIF/dimensions caption drawn under a still-image preview (below the image): `"off"` (image only, no caption), `"basic"` (camera + date), `"essentials"` (+ lens, exposure, ISO, focal length, GPS), or `"full"` (+ software, 35mm-equivalent, EV bias, flash, program, white balance). Unknown values fall back to `"essentials"`. Set from the M-F3 Preview settings dialog. |
| `video_metadata` | bool | `true` | Show the ffprobe metadata line under video thumbnail grids. When `false`, only the thumbnail grid (or "Generating thumbnails…") is shown. Set from the M-F3 Preview settings dialog. |
