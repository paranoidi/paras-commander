...
func fuzzySpans(entry localfs.Entry, colWidth int, ranges []search.Range, highlightCursor bool, styles theme.Theme, showIcons bool, showSize bool, suffix panellist.RowSuffix, scrollbarReserve int, metaW int, nameBGAt func(displayIndex int) tcell.Style) []primitive.Span {
	if len(ranges) == 0 {
		return nil
	}
	nameWidth := nameWidthForColumn(colWidth, showIcons, scrollbarReserve, showSize, metaW)
