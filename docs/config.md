...
| `prefetch_window` | int | `5` | Number of entries fetched ahead of/behind the cursor (clamped 1–50). Entries farther away are never queued for background decode, so they can't evict already-warm near-cursor cache entries. The queue prioritizes entries in the direction the cursor last moved. Ignored when `prefetch` is `false`. |
