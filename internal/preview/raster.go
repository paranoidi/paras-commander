...
func BuildVideoThumbMaxEdgePNG(ctx context.Context, path string, durationSec float64, cols, rows, maxEdge int) ([]byte, error) {
	if maxEdge < 1 {
		maxEdge = 1
	}
	grid, err := buildVideoThumbGrid(ctx, path, durationSec, cols, rows, maxEdge, maxEdge)
	if err != nil {
		return nil, err
	}
	var buf bytes.Buffer
