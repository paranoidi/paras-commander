...
func TestActionFromKeyMapsNavigationKeys(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := lookupActionForView(event, km, nil, nil, nil, nil, ui.ViewBrowser)
			if got != tt.want {
...
func TestActionFromKeyMapsQuitImmediate(t *testing.T) {
	...
	got := lookupActionForView(ev, km, nil, nil, nil, nil, ui.ViewBrowser)
...
func TestActionFromKeyMapsCtrlCToJobsCancel(t *testing.T) {
	...
	if got := lookupActionForView(event, bundle.Global, bundle.Jobs, bundle.Commands, bundle.Messages, bundle.FilePreview, ui.ViewJobs); got != keymap.ActionJobsCancel {
		t.Fatalf("jobs view Ctrl+C = %v, want ActionJobsCancel", got)
	}
	if got := lookupActionForView(event, bundle.Global, bundle.Jobs, bundle.Commands, bundle.Messages, bundle.FilePreview, ui.ViewBrowser); got != keymap.ActionPanelToggleCarousel {
...
func TestActionFromKeyMapsF3F4(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := lookupActionForView(tt.ev, km, nil, nil, nil, nil, ui.ViewBrowser); got != tt.want {
...
func TestLookupF8BrowserVsJobsOverlay(t *testing.T) {
	...
	if got := lookupActionForView(f8, bundle.Global, bundle.Jobs, bundle.Commands, bundle.Messages, bundle.FilePreview, ui.ViewBrowser); got != keymap.ActionFileDelete {
		t.Fatalf("browser F8 = %q, want file.delete", got)
	}
	if got := lookupActionForView(f8, bundle.Global, bundle.Jobs, bundle.Commands, bundle.Messages, bundle.FilePreview, ui.ViewJobs); got != keymap.ActionJobsTerminate {
...
func TestLookupF8BrowserVsMessagesOverlay(t *testing.T) {
	...
	if got := lookupActionForView(f8, bundle.Global, bundle.Jobs, bundle.Commands, bundle.Messages, bundle.FilePreview, ui.ViewMessages); got != keymap.ActionMessagesClear {
